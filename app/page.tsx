...
export default async function Home({ searchParams }: { searchParams: Promise<{ domain?: string | string[]; q?: string | string[] }> }) {
  const [params, lessons, user] = await Promise.all([searchParams, getPublishedLessons(), getChatGPTUser()]);
    ...
    <main>
      <header className="site-header">
        ...
        <nav aria-label="주요 메뉴">
          ...
          {editor ? <Link className="studio-link" href="/studio"><FilePenLine aria-hidden="true" size={16} /> 편집실</Link> : !user ? <a className="studio-link" href={chatGPTSignInPath("/studio")} target="_top"><FilePenLine aria-hidden="true" size={16} /> 편집실</a> : null}
